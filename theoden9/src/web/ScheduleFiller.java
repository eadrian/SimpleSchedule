...
public class ScheduleFiller {
	...
	public List<List<Course>> getRandSchedules(userData udata, TransientData tdata, searchFactors facts, String quarter, int year, int num) {
		List<TransientData> tdatas = getSchedule(udata, tdata, facts, quarter, year);
		List<List<Course>> schedules = new ArrayList<List<Course>>();
		for (int i=0; i<tdatas.size(); i++) {
			schedules.add(tdatas.get(i).courses);
		}
		Random r = new Random();
		int count = 0;
		int attempts = 0;
		...
		System.err.println("Size of results: "+schedules.size());
		
		while (count < num && attempts < MAX_ATTEMPTS) {
			if (schedules.size() <= (num-results.size())) {
			...
			if (!containsSimilar(sched, results, num)) {
				results.add(sched);
		...
		System.err.println("Size of results returned: "+results.size());
		return results;
	}
	
	

	private boolean containsSimilar(List<Course> sched, List<List<Course>> schedules, int num) {
		...
		for (int i=0; i < counts.size(); i++) {
			if (counts.get(i) > ((float)schedules.size() *1f/ 2f) && schedules.size() > (float) num / 2f) {
				System.out.println("Common course rejected");
				return true;
			}
		}
		return false;
	...
	public List<TransientData> getSchedule(userData udata, TransientData tdata, searchFactors facts, String quarter, int year) {
		//System.out.println("printing!!");
		List<TransientData> scheds = new ArrayList<TransientData>();
		/*
		if (tdata.courses.size()>4 || tdata.totalUnits >= TARGET_UNITS) {
			scheds.add(tdata.courses);
			System.out.println("will return");
		...
		*/
		...
		scheds.add(tdata);
		Random r = new Random();
		for (int i=0; i < 5-tdata.courses.size(); i++) {
			List<List<Course>> nextScheds = new ArrayList<List<Course>>();
			
			List<TransientData> nscheds = new ArrayList<TransientData>();
			for (int j=0; j<scheds.size(); j++) {
				int branches = 0;
				TransientData t1 = scheds.get(j);
				
				if (t1.totalUnits >= TARGET_UNITS) {
					nscheds.add(t1);
					continue;
				}
				
				List<Course> results = getBestCourse(udata, t1, facts, quarter, year);
				List<Course> top_results = new ArrayList<Course>();
				for (int k=1; k<TOP_CHOICES; k++) {
					top_results.add(results.get(k));
				}
				Course c = results.get(0);
				...
				for (int k=0; k<results.size(); k++) {
					if (i==0)
						System.out.println("RAND: "+rand);
					if (k==0 || (rand < 5 && branches < MAX_BRANCH && scheduleCount <MAX_SCHEDULES )) {
						if (k>0) {
							branches++;
							scheduleCount++;
							branchCount++;
						}
						Course chosen = results.get(k);
						
						TransientData t = new TransientData(t1);
						t.barredCourses.add(chosen.code);
	...
	public List<Course> getBestCourse(userData udata, TransientData tdata, searchFactors facts, String quarter, int year) {
		...
		float expecGERS = (float)tdata.GERSneeded.size() / quartersLeft - tdata.GERSTaken;
		
		float expecMajor = (float)tdata.reqs.sortedReqs.size() / quartersLeft - tdata.REQSTaken;
		...
		for (int i=0; i<allResults.size(); i++) {
			...
			*/
			...
			if (!GERS.equals(null)) {
				...
				for (int j=0; j<gersTaken.length || j==0; j++) {
					if (tdata.GERSneeded.contains(gersTaken[j])) {
						...
						c.totalScore += (urgency*(MAX_SCORE*expecGERS));
			...
			if (tdata.courseExpec.containsKey(c.deptCode)) {
				...
				if (tdata.expecDept.containsKey(c.deptCode) && !tdata.deptsTaken.containsKey(c.deptCode))
			...
			//Prereqs
			c.totalScore+=((float)quartersLeft*4)/MAX_QUARTERS * c.preScore;
			...
			*/
			...
			Random r = new Random();
			c.totalScore+=r.nextInt((int) MAX_SCORE/2);
		}
