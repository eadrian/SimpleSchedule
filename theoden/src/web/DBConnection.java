...
public class DBConnection {
	...
	//Check course
	public boolean courseExists(String table, String code, String quarter) {
		try {
			Statement stmt = con.createStatement();
			stmt.executeQuery("USE " + database);
			ResultSet rs = stmt.executeQuery("SELECT * FROM "  + table + " WHERE code=" + code + " AND quarter='"+quarter+"';" );
			while(rs.next()){
				return true;
			}				
			return false;
