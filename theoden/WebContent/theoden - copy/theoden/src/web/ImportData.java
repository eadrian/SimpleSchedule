...
public class ImportData {
	...
	private static void importCourse(String classNumStr, String avgGrade, String code, String description, 
		...
		String sep = "\", \"";		
		String query = "";
		if (dbc.courseExists("rhun_courses", code, "Autumn")) {
	...
	public static void importLecturers(String classNum, List<String> lecturersList, String quarter) {
		
		for (int i = 0; i < lecturersList.size(); i++) {	
			String lecturerName = lecturersList.get(i);		
			int star = 0;
			if (lecturerName.contains("*")) {
				star = 1;
				lecturerName = new String(lecturerName.substring(1));
			}
	...
	/*** HELPER FUNCTIONS ***/
	public static int getDepartmentID(String code, String name) {			// gets the existing department ID from the database
